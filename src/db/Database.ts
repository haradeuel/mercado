...
export class Database {
  ...
  private static async criarConexao(): Promise<void> {
    this.pool = new Pool({
  ...
  public static async getConexao(): Promise<Pool> {
    if (!this.pool) {
      await this.criarConexao(); // Cria a conexão se não houver uma ativa
    }
